            ...
            <div className="grid grid-cols-2 gap-4 order-1 lg:order-2">
              {highlights.map((item, index) => (
                <div key={index} className="bg-gray-800/50 p-4 lg:p-6 rounded-lg hover:bg-gray-800 transition-all duration-300 hover:scale-105">
                  ...
                  <h3 className="text-sm lg:text-lg font-semibold text-white mb-2">{item.title}</h3>
                  <p className="text-xs lg:text-sm text-gray-400 leading-relaxed">{item.description}</p>
                </div>
              ))}
            </div>
          </div>
